"""
Binning Functions for creating WOE Binning based on woeBinningPandas package with slight modifications
"""
...
def woe_binning2 (df, target_var, pred_var, min_perc_total, min_perc_class, stop_limit, abbrev_fact_levels, bad, good):
    """
...
def woe_binning3(df, target_var, pred_var, min_perc_total, min_perc_class, stop_limit, abbrev_fact_levels, bad, good):
    """
    ...
    # Build Subsets with target and predictor variable
    df = df[[target_var, pred_var]]
    dfrm = df[[target_var, pred_var]]
    dfrm.columns = ['target_var', 'predictor_var']
    
    # Binning in case of a numerical variable was selected
    if len(dfrm.iloc[:, 0].drop_duplicates()) == 2 and (dfrm.iloc[:, 1].dtypes.kind in 'bifc') == True:
        ...
        # Derive cutpoints for bins (with similar frequency)
        cutpoints = dfrm.predictor_var.quantile(np.arange(0, max_bins+1)/max_bins).reset_index(drop=True)
        innercutpoints = [-np.inf] + list(cutpoints[1:len(cutpoints)-1]) + [np.inf]
        cutpoints = list(dict.fromkeys(innercutpoints))
        cutpoints.sort()
        return(cutpoints)